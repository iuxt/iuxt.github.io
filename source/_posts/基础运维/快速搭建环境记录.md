...
```bash
# 下载解压,创建软连接
curl -OL -C - https://s3.babudiu.com/src/linux/tar/jdk-8u461-linux-x64.tar.gz
...
```bash
curl -OL -C - https://dl.google.com/go/go1.23.1.linux-amd64.tar.gz
...
# 下载安装包
curl -OL -C - "https://npmmirror.com/mirrors/node/${NODE_VERSION}/node-${NODE_VERSION}-linux-x64.tar.xz"
